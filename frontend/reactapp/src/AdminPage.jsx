                    ...
                    {!expandedUserId && (
                        <div className="flex-grow-1 mx-md-4 w-100 mb-3 mb-md-0" style={{maxWidth: '400px'}}>
                            <div className="input-group">
                                <span
                                    className="input-group-text bg-white border-end-0 border-secondary-subtle rounded-start-pill ps-3">
                                    ...
                                    <div key={u.id} className="col-12 col-md-6 col-lg-4 col-xl-3">
                                        <div
                                            ...
                                            style={{
                                                ...
                                                opacity: u.active ? 1 : (isPending ? 0.85 : 0.65)
                                            }}
                                            onClick={() => {
                                                if (!isPending) toggleUserExpansion(u.id);
                                            }}
                                            onMouseEnter={(e) => {
                                                if (!isPending) {
                                            ...
                                            <div className="mb-3 position-relative d-inline-block mx-auto">
                                                <img
                                                    ...
                                                    style={{
                                                        ...
                                                        height: '80px',
                                                        objectFit: 'cover',
                                                        filter: u.active ? 'none' : (isPending ? 'grayscale(50%)' : 'grayscale(100%)')
                                                    }}
                                                />
                                            </div>

                                            <h5 className={`fw-bold mb-1 text-truncate ${isInactive ? 'text-muted text-decoration-line-through' : ''}`}
                                                style={{color: u.active || isPending ? '#16264A' : ''}}>
                                                {u.firstName} {u.lastName}
                                            </h5>
                                            <p className="text-muted small mb-2 text-truncate">@{u.username}</p>

                                            <div className="mb-3 d-flex justify-content-center align-items-center">
                                                <span className="badge px-3 py-2 me-1" style={{
                                                    backgroundColor: u.admin ? '#2D5A88' : '#e2e8f0',
                                                    color: u.admin ? 'white' : '#475569'
                                                }}>
                                                    {u.admin ? t('adminPage.roleAdmin') : t('adminPage.roleStandard')}
                                                </span>
                                                {isInactive && (
                                                    <span className="badge px-3 py-2 ms-1"
                                                ...
                                                <div className="mt-auto pt-3 border-top">
                                                    <button
                                                        ...
                                                        onClick={async (e) => {
                                                            e.stopPropagation();
                                                            if (u.active) {
                                                                if (window.confirm(t('adminPage.confirmDeactivateUser', {name: u.firstName}))) {
                                                                    await deleteUser(u.id, false);
                                                                    // Fetch the current page again to refresh the UI
                                                                    fetchUsersPaginated(currentPage, itemsPerPage, userSearchTerm);
                                                                }
                                                            } else {
                                    ...
                                    <ul className="pagination modern-pagination mb-0">
                                        <li className={`page-item ${currentPage === 1 ? 'disabled' : ''}`}>
                                            <button className="page-link"
                                                    onClick={() => setCurrentPage(p => Math.max(1, p - 1))}>&laquo;</button>
                                        </li>
                                        ...
                                        <li className={`page-item ${currentPage === Math.ceil(totalUsersCount / itemsPerPage) ? 'disabled' : ''}`}>
                                            <button className="page-link"
                                                    onClick={() => setCurrentPage(p => Math.min(Math.ceil(totalUsersCount / itemsPerPage), p + 1))}>&raquo;</button>
                                        </li>
                                    </ul>
                                </nav>
                            </div>
                            ...
                            return (
                                <div>
                                    <div className="glass-card mb-4 border-0 overflow-hidden d-inline-block pe-md-5">
                                        <div
                                            className="d-flex flex-column flex-md-row align-items-center align-items-md-start p-4"
                                            style={{background: 'linear-gradient(to right, rgba(45, 90, 136, 0.05), rgba(255, 255, 255, 0.5))'}}>
                                            ...
                                            {(() => {
                                                ...
                                                return (
                                                    <>
                                                        <div
                                                            className="position-relative me-md-5 mb-3 mb-md-0 flex-shrink-0">
                                                            <img
                                                                src={u.photoUrl || '/default-user.png'}
                                                                alt="User Profile"
                                                        ...
                                                        <div
                                                            className="text-center text-md-start me-md-5 mb-3 mb-md-0 mt-md-2">
                                                            <h3 className={`fw-bold mb-1 ${isInactive ? 'text-muted text-decoration-line-through' : ''}`}
                                                                style={{color: u.active || isPending ? '#16264A' : ''}}>
                                                                {u.firstName || t('adminPage.pendingUser')} {u.lastName}
                                                            </h3>
                                                            <p className="text-muted mb-2">@{u.username}</p>
                                                            <div
                                                                className="d-flex align-items-center justify-content-center justify-content-md-start mt-2">
                                                                <span className="badge px-3 py-2 me-1" style={{
                                                                    backgroundColor: u.admin ? '#2D5A88' : '#e2e8f0',
                                                                    color: u.admin ? 'white' : '#475569'
                                                                }}>
                                                                ...
                                                                {isInactive && (
                                                                    <span className="badge px-3 py-2 ms-1" style={{
                                            ...
                                            <div
                                                className="d-flex flex-column align-items-center align-items-md-start text-muted small mt-md-3 pt-md-1">
                                                ...
                                                <div className="px-4 pb-4 pt-0">
                                                    {sortedSelectedClients.length > 0 ? sortedSelectedClients.map(c => (
                                                            <div key={c.id}
                                                                ...
                                                                <span
                                                                    className={`fw-semibold text-truncate ${!c.active ? 'text-muted text-decoration-line-through' : ''}`}
                                                                    title={c.name}>
                                                                    {c.name}
                                                                </span>
                                                                    <div
                                                                        className="d-flex align-items-center mt-1 text-muted text-truncate"
                                                ...
                                                <div
                                                    ...
                                                    {selectedUserLeads.length > 0 && (
                                                        <select
                                                            ...
                                                            <option
                                                                value="2">{t('adminPage.stateProposalSent')}</option>
                                                ...
                                                <div className="px-4 pb-4 pt-0">
                                                    {filteredAndSortedLeads.length > 0 ? filteredAndSortedLeads.map(l => (
                                                            <div key={l.id}
                                                                ...
                                                                <div className="text-start d-flex flex-column pe-2"
                                                                     style={{flex: '0 0 45%', minWidth: 0}}>
                                                                    <span
                                                                        className={`fw-semibold text-truncate ${!l.active ? 'text-muted text-decoration-line-through' : ''}`}
                                                                        title={l.title}>{l.title}</span>
                                                                    <span className="text-muted text-truncate mt-1"
                                                                          style={{fontSize: '0.75rem'}}
                                                                          title={l.description}>
                                                                    {l.description ? l.description :
                                                                        <i className="opacity-50">No description</i>}
                                                                </span>
                                                                </div>
                                                                <div style={{flex: '0 0 30%'}} className="text-center">
                                                                <span className="badge border text-muted fw-normal"
